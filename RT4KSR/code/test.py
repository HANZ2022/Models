...
def test(config):
    ...
    for benchmark in config.benchmark:
        ...
        with torch.no_grad():
            print("Testing...")
            os.makedirs(f"../Output/{benchmark}_X{config.scale}/RT4KSR/results/SR2", exist_ok=True)
            for batch in tqdm(test_loader):
                ...
                print(out.shape)
                pad_h = batch.get("pad_h", 0)
                pad_w = batch.get("pad_w", 0)
                print(pad_h,pad_w)
                if pad_h > 0 or pad_w > 0:
                    # Remove padding before saving
                    h, w, _ = out.shape
                    out = out[:h - pad_h * config.scale, :w - pad_w * config.scale, :]
                    print(f"Removed padding: new shape {out.shape}")

                '''Save image'''

                # Get the image name from the batch (assuming the dataloader provides this)
                img_name = batch.get("name", f"img_{len(test_results['psnr_rgb'])}")
                # Save the output image
                raw_name = batch["name"][0]  # This gives you the string with brackets and quotes
                clean_name = raw_name.replace("[", "").replace("]", "").replace("'", "")
                save_filename = f"../Output/{benchmark}_X{config.scale}/RT4KSR/results/SR2/{clean_name}.png"
                print(save_filename)
                Image.fromarray(out).save(save_filename)
