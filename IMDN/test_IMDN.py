...
for imname in filelist:
    ...
    img_path = opt.test_lr_folder + imname.split('/')[-1].split('.')[0] + ext
    #img_path = opt.test_lr_folder + imname.split('/')[-1].split('.')[0] + 'x' + str(opt.upscale_factor) + ext
    print(img_path)
    im_l = cv2.imread(img_path, cv2.IMREAD_COLOR)[:, :, [2, 1, 0]]  # BGR to RGB
